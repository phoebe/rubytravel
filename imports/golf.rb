...
def initialize() 
    @info = {  "feature_code" => "GOLF", "source" => "DBP", "use_code"=>"golf" }
    @url='http://dbpedia.org/'
	# avoid yplookup, do not overload existing record
    @options={"yptype"=>"Golf Courses","soft"=>true,"reallysoft"=>true}
	@collect=Array.new
...
def allResorts()
	query= @prefix + 'select distinct ?a where { _:c <http://www.w3.org/2004/02/skos/core#broader> category:Golf_clubs_and_courses_in_the_United_States . ?a ?b _:c} order by ?a limit 500 offset 540'
