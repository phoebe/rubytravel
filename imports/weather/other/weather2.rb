...
class Weather < Scraper
	...
	def tally2(file)
		doc = File.open('v2.country.codes')
		countries={}
		begin
		while ( line = doc.readline())
			k=line[0,3].strip
			countries[k]=line[4,40].strip
		end
		rescue EOFError
		end
		doc.close
		...
		begin
		while ( line = doc.readline()) 
			country=line[0,3].strip
			k=line[3,5].strip
			y=line[12,4].strip
			if lastk!=k 
				 q="select name, country_code from stations where alternatenames = '#{ Mysql::escape_string(k) }'"
				 puts q
				 res= @importer.selectquery(q)
				 print	countries[country]+' : ' unless countries[country].nil?
				 res.each { |ans| puts ans[0]+" "+ans[1] }
				 lastk=k
			end
			ms=[]
			puts "#{k} #{y} "
			(0..11).each { |m|
				ms[m]= (line[16+m*5, 5].to_i)/10.0
				print "#{ms[m]} "
			}
			puts ""
			i=i+1
