<!-- -*- mode: indented-text;-*- -->
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:uri="http://www.w3.org/2000/07/uri43/uri.xsl?template=" xmlns:html="http://www.w3.org/1999/xhtml" exclude-result-prefixes="uri html">
...
<p>Copyright (c) 2000 W3C (MIT, INRIA, Keio), released under the <a href="http://www.w3.org/Consortium/Legal/copyright-software-19980720">
...
<xsl:variable name="uri:lowalpha" select="&quot;abcdefghijklmnopqrstuvwxyz&quot;"/>
<xsl:variable name="uri:upalpha" select="&quot;ABCDEFGHIJKLMNOPQRSTUVWXYZ&quot;"/>
<xsl:variable name="uri:digit" select="&quot;01234567890&quot;"/>
<xsl:variable name="uri:alpha" select="concat($uri:lowalpha, $uri:upalpha)"/>
...
<xsl:template name="uri:expand">
  ...
  <xsl:variable name="rest" select="substring($there, 1,              string-length($there)       - string-length($hashFragment))"/>
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
	  ...
	  <xsl:value-of select="$there"/>
        ...
        <xsl:otherwise>
	  <xsl:variable name="rest2" select="substring($rest, string-length($scheme) + 1)"/>
	  ...
	  <xsl:choose>
	    <xsl:when test="starts-with($rest2, &quot;//&quot;)">
	      ...
	      <xsl:value-of select="concat($baseScheme, &quot;:&quot;,         $rest2, $hashFragment)"/>
            </xsl:when>

	    <xsl:otherwise>

	      <xsl:variable name="baseRest" select="substring($base,      string-length($baseScheme) + 2)"/>
		...
		<xsl:call-template name="uri:split-authority">
		  ...
		  <xsl:value-of select="concat($baseScheme, &quot;:&quot;,             $baseAuthority,             $rest2, $hashFragment)"/>
		</xsl:when>
		...
		<xsl:otherwise>
		  ...
		  <xsl:variable name="baseRest2" select="substring($baseRest,      string-length($baseAuthority) + 1)"/>
		  ...
		  <xsl:value-of select="concat($baseScheme, &quot;:&quot;,             $baseAuthority,             $path,             $hashFragment)"/>
		</xsl:otherwise>
	      </xsl:choose>
	    </xsl:otherwise>
	  </xsl:choose>
...
<xsl:template name="uri:split-scheme">
  ...
  <xsl:variable name="scheme_" select="substring-before($ref, &quot;:&quot;)"/>
  <xsl:choose>
    <!-- test whether $scheme_ is a legal scheme name,
	 i.e. whether it starts with an alpha
	 and contains only alpha, digit, +, -, .
	 -->
    <xsl:when test="string-length($scheme_) &gt; 0             and contains($uri:alpha, substring($scheme_, 1, 1))      and string-length(translate(substring($scheme_, 2),                    concat($uri:alpha, $uri:digit,             &quot;+-.&quot;),             &quot;&quot;)) = 0">
