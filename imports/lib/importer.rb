...
require File.join(File.dirname(__FILE__), './', 'geocoder_google')
require 'mysql'
require 'logger'
...
# use_code is special - manual - do not write over, but update by adding tags
class Importer
  def initialize(preset)
    @places_textfields=%w(name asciiname alternatenames feature_class feature_code feature2_code country_code  street_address city state postal_code phone email  url source source_id open_days hours admin1_code transportation transportation_note note thumbnail_link use_code )
    @places_numericfields=%w( geonameid latitude longitude elevation maxelevation hour_start hour_end difficulty distance area howaccessible parent rating)
	$0=~/([^\/]+).rb/
    $log = Logger.new("log.#{$1}.txt",'daily') if $log.nil?
  ...
  def findWeather(par={})
    ...
    wid=nil;
    q = 'select * from weatherInfo where geonameid = '+par['geonameid']
    begin
      res=@dbh.query(q);
      if (res.num_rows > 0)
        res.each_hash { |row| 
  ...
  def insertWeather( clim, par)
    geonameid= findWeather(par)
    unless geonameid.nil? #  already inserted
  ...
  def fetchRecord(par)
    if (( par['name'].blank? && par['city'].blank? ) && par['id'].blank?  && par['source_id'].blank? )
    ...
    begin
      str=""
      %w(name city).each { |f|
        str= str + " and "+f+'="'+ Mysql::escape_string(par[f])+'"' if (!par[f].blank? )
      }
  ...
  def fixAddress
	...
	res.each { |d|
		if d[0] =~ /(\S.*\S)\s*(http:\S+)/
			q= "update places set street_address='#{  Mysql::escape_string($1) }' where id=#{d[5]};"
			insertIntoDB(q)
  ...
  def locationExists?(h)
    ...
    %w(name country_code ).each { |f|
      str= str + f+' = "'+Mysql::escape_string(h[f])+ '" and ' unless h[f].blank?
    }
    f=state
    str= str+' admin1_code = "'+Mysql::escape_string(h[f])+ '" and ' unless h[f].blank?
...
=end
  ...
  def fillCoord(h)
    coord= @coder.glookup( h['street_address'], h['city'], h['state'], h['postal_code'],h['country_code']);
    unless coord.blank?
  ...
  def insertPlaceIntoDB(h)
    ...
    @places_numericfields.each { |f|
      unless h[f].blank? 
        ...
        valuelist=valuelist+ ','+ Mysql::escape_string( h[f].to_s )
      end
    }
  ...
  def reallyEnterIntoDB( h, options={}) 
    #begin
      nolookup=false	# nolookup means take the address/coords as is
      soft=false	# soft means enter even if not found in YP
	  reallysoft=false
	  unless options.blank?
		  soft= !options['soft'].blank? 
		  reallysoft=  !options['reallysoft'].blank? 
		  nolookup= !options['nolookup'].blank? 
	  end
      return if h.blank? 
      ...
      if (!h['street_address'].blank? && !h['city'].blank? && !h['state'].blank?  ) # has address
		  ...
		  elsif !nolookup && fillCoord(h) # if address is bad, don't insert
            sleep(1) if insertPlaceIntoDB(h)
          end
					 # missing street address, but has city and valid url 
      elsif ( !h['url'].blank? && !h['city'].blank? && !h['name'].blank? && soft )
          puts  " name/city/url is good enough #{h['name']} #{ h['city']} #{h['url']} "
		  fillCoord(h) unless !h['latitude'].nil? || nolookup    # find the coords if missing
		  sleep(1) if insertPlaceIntoDB(h) # if this fails
      elsif ( !h['name'].blank? && !h['country_code'].blank? &&  reallysoft )
		  sleep(1) if insertPlaceIntoDB(h) # if this fails
      else
          puts "Fail to insert #{h['name']} #{ h['city']}"
      end
    #rescue => e
  ...
  def compareHashes(h,rec, options={} )
    ...
    if !options.blank? 
      mode=0 if options['mode']=='add'    # in h, but not in db, ignore null - default
      mode=1 if options['mode']=='sub'    # in db, but not in h, ignore null
  ...
  def comparePlaceInDB(h, options={} )
	...
	str="";
	str = str+ h['use_code'].downcase unless h['use_code'].nil? 
	str = str +','+ rec['use_code'].downcase unless rec['use_code'].nil?
	str.chomp(',');
	array= str.split(',');
	unless array.empty?
		array.uniq! 
    ...
    unless diff['street_address'].blank? &&  diff['city'].blank? &&  diff['state'].blank? # new address ? 
      h2=h.dup
      if !overload 
		  if options['nolookup'].blank?  # do lookups
			add=yplookup(h2, options) if !options['nolookup'].blank?# do a lookup in yellow pages to check address
  ...
  def amendAddress(h, options={})
	  return if h.blank?
	  h2={};
		Geocoder::greverselookup(h['latitude'],h['longitude'],h2)
		h['country_code']=h2['country_code'];
		h['city']=h2['city'] if h['city'].blank?;
		h['state']=h2['state'] if h['state'].blank?;
		h['street_address']=h2['street_address'] if h['street_address'].blank?;
  end
  ...
  def InsertorUpdatePlaceInDB(h, options={})
    return if h.blank?
    h.each{|k,v|
      v.to_s.chomp!(',') #  hate those trailing commas
    }
    known= false;
	# use latlng to fill in missing address components
	amendAddress(h,options) if (h['country_code'].nil? || h['city'].blank? || h['state'].blank?) && !h['latitude'].blank?
    if recordExists?(h) # found right away
      known=true
      add=h
    else          #  do a look up
	  if (options['nolookup'].nil?)
		  h2= h.dup
		  add= yplookup(h2, options)   # use yellow pages to check address or add number
	 	  if ( add.blank? )
			if h2['city'].blank?
			  h2.delete("city")
			  sleep 1;
    ...
    if known
      fields = comparePlaceInDB(add) # returns id and all fields that are different
		puts "Update fields:  "+ fields.inspect
  ...
  # do later
  def updateFrom(par,fromtable,fromid,totable,toid)
  test1="update #{totable}
	set #{f} = #{par[f]}
	  from #{fromtable} a 
	where #{totable}.#{f} is null
	and exists (
	  select *
	  from #{fromtable} b
	  where a.#{fromid} = b.#{toid}
  )"
  test2="UPDATE A
  ...
  def updatePlaceInDB(h,options={})
    ...
    str="set"
    @places_textfields.each{ |f|
      str= str + " "+f+'="'+ Mysql::escape_string( h[f] )+'",' unless ( h[f].blank? )
    }
    @places_numericfields.each{ |f|
      str= str + " "+f+'='+ Mysql::escape_string( h[f].to_s )+',' unless ( h[f].blank? )
    }
    str.chomp!(',')
    begin
...
def test1
  ...
  'feature_code' => 'WINE',
  'country_code'=>'US']
  h=Hash[ 'name'=>'Stone Cottage Cellars',
 'street_address'=>'41716 Reds Rd.',
 'use_code'=>'wine tasting',
 'feature_code' => 'WINE',
 'city'=>'Paonia']
  p= Importer.new('')
  rec= p.fetchRecord(h)
...
def test_url
  p= Importer.new('')
  # one.dom
  #%w( http://www.squaw.com/ http://www.amwellvalleyvineyard.com/  http://www.plumcreekwinery.com http://www.chiff.com/wine/winery.htm  http://www.target.com/gp/detail.html/602-4045909-4263801?ASIN=B000NPCK3W&AFID=Froogle&LNM=B000NPCK3W|Lexmark_AllInOne_Printer_with_Scanner_and_Copier__X1240&ci_src=14110944&ci_sku=B000NPCK3W&ref=tgt_adv_XSG10001 http://www.domain.com one.dom ).each { |url|
  %w( http://www.squaw.com/ http://www.amwellvalleyvineyard.com/  http://www.plumcreekwinery.com http://www.domain.com one.dom ).each { |url|
    puts url
    puts 'OK' if (p.UrlAvailable?(url))
  }
end

def self::constructQueryWithList(h,table,stringlist,numlist)
	fstr='';
	vstr='';
	...
	fstr.chomp!(',')
	vstr.chomp!(',')
	return 'insert into table '+table+' ('+fstr+') values ('+vstr+')';
end
