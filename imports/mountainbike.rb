...
class Resort < Scraper
def initialize() 
    @info = {  "feature_code" => "MTB","nolookup"=>true, "source" => "DBP", "use_code"=>"mountainbike" }
...
def allResorts()
	#category:Mountain_biking_venues_in_the_United_States
	query= @prefix + 'select distinct ?a where { ?a skos:subject
	     category:Mountain_biking_venues_in_Canada
	} order by ?a limit 500 offset 12'
...
def makequery(uri, *args)
	q= ' select distinct * where  {'+ uri+
	  ...
	  ' optional {'+ '{'+ uri+ ' dbpedia-owl:minElevation ?min } '+
      ' union {'+ uri+ ' dbpedia-owl:elevation ?min } '+
	  '} . '+
	  ' optional {'+ uri+ ' dbpedia-owl:maxElevation ?max .} .'+
