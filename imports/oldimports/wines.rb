  ...
  def oregonwine2(httppage)
    ...
    while ( i+4 <= max ) do
      if ( textf[i+3].text =~ /Yes/i )
        ...
        #puts "[#{name}] [#{textf[i]} [#{textf[i+1]} [#{textf[i+2]} [#{textf[i+3]} ] ";
        #sleep(1) unless ! p.from_yp( name, a['town'],'OR','USA','S','WINE','CAW',a['url']);
        # h.each { |k,v| puts "[#{k}] [#{v}]" }
      end
      i=i+4
    end
  ...
  def parseAddress(doc,h) 
    doc.search('//tr').each { |tr|
      ...
      if ( td[0].text()=~/Location:/i ) 
        h['street_address'] = (td[1].search( "./br/preceding-sibling::text()")).to_s
        temp = ( td[1].search("./br/following-sibling::text()")).to_s;
        if (temp =~ /^(.+), \w{2}\s+([-\d]+)$/im)
          h['city']= $1.strip
          h['state']= 'CO'
          h['zip']= $2.strip
        ...
        page.noko.css('a').each { |l|
          ...
          if (h['info'] =~ /wineryDetail.cfm/) 
            ...
            @agent.transact do
              ...
              add = parseAddress(wine_page.noko, h) 
              puts add.inspect
              sleep (5)
              sleep(1) if @importer.insertPlaceIntoDB(add)
            end
...
class TXwines
  def initialize()
    ...
    @subp=%w(findawinery/central.html findawinery/northern.html findawinery/southeast.html findawinery/western.html)
  end

  def crawlAll() 
    info= {"country_code"=>"US","state"=>"TX","admin1_code"=>"TX","source"=>"TXW"}
    doc = Nokogiri.parse(File.open('southeast.html'));
    parseAll( doc, page, info )
  end

  def parseAll( doc, page, info) 
      doc.css('div[id="interiorContent"]').each { |win|
        win.css('h2 a').each { |link|
        }
      }
  end
