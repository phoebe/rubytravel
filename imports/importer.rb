...
=end
  ...
  def reallyEnterIntoDB( h, options={}) 
    #begin
      ...
      if (!h['street_address'].blank? && !h['city'].blank? && !h['state'].blank?  ) # has address
          unless ( !nolookup && fillCoord(h)==nil   )  # if address is bad, don't insert
		  ...
		  fillCoord(h) unless nolookup    # find the coords
  ...
  def comparePlaceInDB(h, options={} )
    ...
    unless diff['street_address'].blank? &&  diff['city'].blank? &&  diff['state'].blank? # new address ? 
      h2=h.dup
      if !overload 
        add=yplookup(h2, options) unless options['nolookup'].blank?# do a lookup in yellow pages to check address
